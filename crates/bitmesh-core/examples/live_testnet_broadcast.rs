...
//! - The seed is read from `NIMIQ_BITMESH_TESTNET_SEED` (hex) or freshly randomised; it
//!   lives only in this process and is never broadcast or sent over FFI.
//! - The RPC URL is injectable via `NIMIQ_BITMESH_TESTNET_RPC_URL` (defaults to the public
//!   testnet endpoint).
...
//! - `NIMIQ_BITMESH_TESTNET_RPC_URL`  testnet JSON-RPC base (default public endpoint)
//! - `NIMIQ_BITMESH_TESTNET_SEED`     32-byte hex seed (reuse a funded key across runs)
//! - `NIMIQ_BITMESH_RECIPIENT`        `NQ…` recipient (default: send to self)
//! - `NIMIQ_BITMESH_VALUE_LUNA`       amount in luna (default 100000 = 1 NIM)
//! - `NIMIQ_BITMESH_FAUCET_URL`       testnet faucet endpoint (default tapit)
//! - `NIMIQ_BITMESH_USE_FAUCET=1`     opt in to tapping the faucet for this address
...
use bitmesh_core::nimiq::signer::{AppSigner, EnclaveKey, InMemoryEnclaveKey, TransferIntent};
use bitmesh_core::rpc::{GatewayRpc, HttpGatewayRpc, DEFAULT_TESTNET_RPC_URL};
...
fn resolve_seed() -> Result<[u8; 32], Box<dyn Error>> {
    if let Some(hex) = env("NIMIQ_BITMESH_TESTNET_SEED") {
        let bytes = hex_to_bytes(hex.trim())?;
        let seed: [u8; 32] = bytes
            .try_into()
            .map_err(|_| "NIMIQ_BITMESH_TESTNET_SEED must be exactly 32 bytes (64 hex chars)")?;
        println!("  seed: loaded from NIMIQ_BITMESH_TESTNET_SEED");
        Ok(seed)
    } else {
        let mut seed = [0u8; 32];
        getrandom::getrandom(&mut seed)?;
        println!("  seed: freshly randomised (set NIMIQ_BITMESH_TESTNET_SEED to reuse it)");
        println!(
...
fn tap_faucet(rpc: &HttpGatewayRpc, address: &str) -> Result<(), Box<dyn Error>> {
    let faucet = env("NIMIQ_BITMESH_FAUCET_URL").unwrap_or_else(|| DEFAULT_FAUCET_URL.to_string());
...
fn main() -> Result<(), Box<dyn Error>> {
    println!("== nimiq.bitmesh G8 — live TESTNET broadcast (money-path, testnet only) ==");
    ...
    let rpc_url =
        env("NIMIQ_BITMESH_TESTNET_RPC_URL").unwrap_or_else(|| DEFAULT_TESTNET_RPC_URL.to_string());
    ...
    // --- 3. optional faucet -----------------------------------------------------------
    if env("NIMIQ_BITMESH_USE_FAUCET").as_deref() == Some("1") {
        tap_faucet(&rpc, &address)?;
    } else {
        println!("\n[3/6] faucet: skipped (set NIMIQ_BITMESH_USE_FAUCET=1 to tap it)");
        if let Ok(Some(acct)) = rpc.get_account(&address) {
    ...
    let recipient = env("NIMIQ_BITMESH_RECIPIENT").unwrap_or_else(|| address.clone());
    let value: u64 = env("NIMIQ_BITMESH_VALUE_LUNA")
        .map(|v| v.parse())
        .transpose()?
