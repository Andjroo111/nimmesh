...
use crate::swap_mirror::describe_verify;
use crate::swap_wire::{
...
// The observable-mirror types moved to their natural home (`swap_mirror`) for the 800-line
// guard; re-exported so `crate::swap_session::{SwapMirror, SwapVerifyNote, telemetry_now_ms}`
// callers (engine, tests, swap_claim) keep their paths.
pub(crate) use crate::swap_mirror::{telemetry_now_ms, SwapMirror, SwapVerifyNote};
...
pub struct SwapSession {
    ...
    pub(crate) verifier: Box<dyn FundingVerifier>,
    ...
    pub(crate) confirm_policy: ConfirmationPolicy,
    ...
    pub(crate) counterparty_chain: Asset,
    ...
    pub(crate) verify_notes: HashMap<[u8; SWAP_ID_LEN], SwapVerifyNote>,
...
impl SwapSession {
    ...
    /// Production rides [`fast_due`](Self::fast_due) + [`reverify_awaited_funding`] directly
    /// (the fast tick also drives the claim ladder); this composed form remains the
    /// deterministic test window into the gate.
    #[cfg(test)]
    pub(crate) fn fast_reverify(&mut self, now_ms: u64) -> Vec<[u8; SWAP_ID_LEN]> {
        if !self.fast_due(now_ms) {
            return Vec::new();
        }
        self.reverify_awaited_funding()
    }

    /// Gate the fast heartbeat: `true` when in-flight settlement work EXISTS — a proof-seen swap
    /// awaiting counterparty funding, or (run-4 fix) a responder with claim work (an unbroadcast
    /// claim to attempt / a broadcast claim awaiting confirmation) — AND the rate limiter grants
    /// a slot at `now_ms`. Checked in that order so an idle poll never burns the slot.
    pub(crate) fn fast_due(&mut self, now_ms: u64) -> bool {
        let awaiting = self.funding_proof_seen.iter().any(|id| {
            self.coordinators
                .get(id)
                .is_some_and(|c| c.awaits_counterparty_funding())
        });
        (awaiting || self.has_claim_work()) && self.fast_verify.due(now_ms)
    }
    ...
    pub fn tick(&mut self, head: u64) -> Vec<[u8; SWAP_ID_LEN]> {
        ...
        self.prune_dropped_swap_state();
        // Run-4 fix: AFTER the reap, foreclose any responder whose inbound claim window has
        // closed unconfirmed (`Revealed`, head past `T_A`) → the honest `Lost` terminal — but
        // only after ONE last-chance claim-confirmation consult, so a claim the chain shows
        // buried settles instead of being mislabeled a loss at the boundary. Placed after the
        // retain so a freshly-Lost (or freshly-settled) swap is mirrored for one tick (the
        // operator SEES the outcome) before the next tick reaps it.
        let expired: Vec<[u8; SWAP_ID_LEN]> = self
            .coordinators
            .iter()
            .filter(|(_, c)| {
                c.role() == SwapRole::Responder
                    && c.phase() == SwapPhase::Revealed
                    && head > c.context().terms.nim_timeout
            })
            .map(|(id, _)| *id)
            .collect();
        for id in expired {
            if !self.confirm_claim(&id) {
                if let Some(c) = self.coordinators.get_mut(&id) {
                    let _ = c.forfeit_expired_claim(head);
                }
            }
        }
        stale
