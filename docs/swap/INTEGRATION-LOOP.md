...
- 2026-07-15 · G9 #80 — runtime `MeshNode::stop_advertising()`: withdraws the standing discovery intent WITHOUT `shutdown()`+rebuild, so the app can stop looking for a counterparty while the node and any in-flight swaps keep running (`Job::StopAdvertising` → `SwapSession::stop_advertising`; `readvertise_intent`/`handle_intent` then no-op) · PR #248 · merged (v0.85.0)
- 2026-07-15 · G11 #82 slice — the "no secret hits logs" clause: four `uniffi::Record` configs took raw key material inbound and every one **derived `Debug`** (`intent_seed` = the per-swap secret master → a leaked seed derives `S` for every future swap and pre-claims the counterparty leg; `nim_claim_seed`/`evm_funding_secret` = the NIM-HTLC redeem key + the funded Amoy account). Hand-written redacting `Debug` in a new `ffi_secret_redaction` module (`<redacted N bytes>`, length only) + a suite that fails if a derive returns. No ABI change. ADR-0012 · PR #249 · merged (v0.86.0)
- 2026-07-15 · G11 #82 slice — real entropy, ENFORCED: the live doors took `intent_seed`/`nim_claim_seed` on **length alone**, and `SwapMesh.swift` drew them with a **discarded** `SecRandomCopyBytes` `OSStatus` over a zero-filled `Data(count: 32)` — a failing RNG silently yielded an all-zero seed, from which every `S` is derivable off the cleartext on-wire `swap_id`: **S1 reopened through the app's RNG**. Nothing caught it (every 32-byte value is a valid Ed25519 seed; the secp256k1 secrets were saved only by the accident that zero isn't a valid scalar) — and `live_safety`'s C1 gate **passes** on a zero seed, because a zero-seeded PRF still counts as "not the sim source". C1 asserts *that* you replaced the sim source, not that the replacement has entropy. Fix: `drawSwapSeed()` over UniFFI (Swift now calls it — removes the mode by construction) + `check_seed_entropy` at every door (inside `seed32`, the one chokepoint all 4 secrets funnel through) + a new `swap_secret` module holding the drawer, the gate, the single PRF definition (the two live doors had copy-pasted twins) and `sim_secret` (moved from `swap_node`, name intact for the doclint — which also put `swap_node` back under the 800-line ceiling it sat exactly on). The gate is an SP 800-90B-style health test, NOT an entropy estimator. ADR-0013 · PR #254 · merged (v0.87.0)
