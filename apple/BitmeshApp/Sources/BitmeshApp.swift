...
/// nimiq.bitmesh — the iOS app is a thin native host around two shared pieces:
///   1. the **web `nimiq-ui` layer** (`webui/`), rendered in a `WKWebView` (A1), and
///   2. the **Rust core** (`bitmesh-core` via UniFFI), reached over a JS↔Swift bridge.
